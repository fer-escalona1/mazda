...
if (isset($_POST['entrar'])) {
    ...
    $consulta = "SELECT * FROM aministradores WHERE ClaveAmin = '$rclave' AND Contraseña = '$rpassword'";
    $claveok = null;
    $passok = null;

    if ($resultado = $connect->query($consulta)) {
        if ($row = $resultado->fetch_assoc()) {
            $claveok = $row["ClaveAmin"];
...
<body>
    <div class="container mt-5">
        ...
        <h2 class="text-center mb-4">Inicia Sesión ADMIN</h2>
        ...
        <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST">
            <!-- Campo de Clave -->
            <div class="mb-3">
                <label for="clave" class="form-label">Clave de Administrador</label>
                <input 
                    type="text" 
                    class="form-control" 
                    id="clave" 
